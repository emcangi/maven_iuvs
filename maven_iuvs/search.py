...
def build_cache(orbfold=None, cache={}, exclude_xml=True, exclude_dark=True, exclude_txt=True,
    ...
    ----------
    orbfold : dict
                    Metadata for one observation
    cache : dict
            stores all the files along with their orbit folder
    l1c_folder : string
                 Location to look for l1c files.
    Returns
    ----------
    cache : dict
            keys: orbit folder (e.g. 'orbit004500'), values: filenames
    """
    if orbfold:
        orbfolds = [orbfold]
    else:
        orbfolds = [i for i in os.listdir(basefolder) if (os.path.isdir(basefolder + i)) and ('orbit' in i)]
...
def get_from_cache(fn, cache):
    """
    Docstring for get_from_cache
    
    Parameters
    ----------
    fn : string
             Filename for which to find a match in the cache 
    cache : dict
            Cache of filenames by orbit folder generated by build_cache() above
    l1c_folder : string
                 Folder containing orbit subdirectories

    Returns
    ----------
    matchlist[0] : string 
                   Filename of associated l1c
    """


    uid = re.search(uniqueID_RE, fn)[0]
    orbfold = orbit_folder(iuvs_orbno_from_fname(fn))    
    # parent = l1c_folder + orbfold + "/"

    try:
        files_here = cache[orbfold]
        matchlist = [f for f in files_here if (uid in f) and ('.fits.gz' in f)]
        if len(matchlist)==1:
            return matchlist[0]
